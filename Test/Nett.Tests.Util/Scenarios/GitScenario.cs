...
namespace Nett.Tests.Utils.Scenarios
{
    /// <summary>
        ...
        public Config<GitConfig> CreateMergedFromDefaultsWithExplicitFileStore()
        {
            ...
            return Config.CreateAs()
                ...
                .StoredAs(store => store
                    .CustomStore(sys).AsSourceWithName(this.SystemSourceName)
                    .MergeWith().CustomStore(user).AsSourceWithName(this.UserSourceName)
                    .MergeWith().CustomStore(repo).AsSourceWithName(this.RepoSourceName))
                .Initialize();
        }
